...
fun = function _fn( Y/*, NAME*/ ) {
    ...
    var
        unwrap = ko.unwrap,
        ID_PKV_SETTINGS = '#pkv_settings',
        i18n = Y.doccirrus.i18n;

    function applyBindings( model, yNode ) {
        if( ko.dataFor( yNode.one( ID_PKV_SETTINGS ).getDOMNode() ) ) {
            return;
        }
        ko.applyBindings( model, yNode.one( ID_PKV_SETTINGS ).getDOMNode() );
    }

    function cleanNode( yNode ) {
        ko.cleanNode( yNode.one( ID_PKV_SETTINGS ).getDOMNode() );
    }
