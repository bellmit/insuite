 ...
 *
 *  YUI module to render labdata table elements and attach element-specific events
 */
...
YUI.add(
    ...
    function(Y, NAME) {
        ...
        Y.log('Adding renderer for dcforms labdatatable types.', 'debug', NAME);

        /**
         *  Factory method for labadata table element renderers
         *
         *  This is a special version of the table element which allows preset queries on labdata to be mapped
         *  into forms.
         *
        ...
        Y.dcforms.elements.makeLabdataTableRenderer = function(element, creationCallback) {
            ...
            /**
             *  Expand the contents of a single table cell into the subslements which will be drawn
             *
             *  (client-side this is text and maybe a chart)
             *
             *  @param cellTxt
             *  @param xOffset
             *  @param yOffset
            ...
            function getCellSubelements( cellTxt, xOffset, yOffset, cellWidth, align, callback ) {
                var
                    FIT_IMG = 0.9,                      //  leave some space between cells containing images
                    lines = cellTxt.split( '\n' ),
                    chartLine =  '',
                    cellSubElements,
                    i;
                ...
                if ( Y.dcforms.isOnServer ) {
                    //  on server
                    return getCellSubelementsServer( cellTxt, xOffset, yOffset, cellWidth, align, callback );
                }

                for ( i = 0; i < lines.length; i++ ) {
                    if ( 'CHARTMINMAX' === lines[i].substr( 0 , 11 ) ) {
                        chartLine = lines[i];
                        yOffset = yOffset + element.mm.lineHeight;
                        lines[i] = '';
                    }
                }
                ...
                //  add color bar graphic
                if ( '' !== chartLine ) {
                    Y.doccirrus.labdata.utils.createColorBar2Subelem(
                        xOffset, yOffset - element.mm.lineHeight,       //  x, y
                        ( cellWidth * FIT_IMG ), element.mm.lineHeight, //  width, height,
                        element.page.form.zoom,                         //  scaling factor mm -> px
                        chartLine,                                      //  chart properties
                        onColorBarCreated                               //  callback
                    );
                } else {
                    return callback( null, cellSubElements );
                }

                function onColorBarCreated( err, chartSubElem ) {
                    if ( err ) { return callback( err ); }
                    cellSubElements.push( chartSubElem );
                    callback( null, cellSubElements );
                }
            }
             ...
             *  Expand the contents of a single table cell into the subslements which will be drawn
             *
             *  (servcer-side we will need to save an image to disk for HPDF.js to use, and clean it up after)
             *
             *  @param cellTxt
             *  @param xOffset
             *  @param yOffset
            ...
            function getCellSubelementsServer( cellTxt, xOffset, yOffset, cellWidth, align, callback ) {
                var
                    FIT_IMG = 0.9,                          //  leave some space between cells containing images
                    lines = cellTxt.split( '\n' ),
                    chartLine =  '',
            ...
            function generateSubElements( callback ) {
                ...
                function addEmptyNotice( itcb ) {
                    ...
                    var
                        ...
                        defaultMsg = Y.doccirrus.i18n( 'FormEditorMojit.generic.LBL_SELECT_LABDATA' ),
                ...
                function onAllDone( err ) {
                    if ( err ) {
                        Y.log( 'Could not load labdata table: ' + JSON.stringify( err ), 'warn', NAME );
            ...
            function setValue(newValue, callback) {
                //Y.log('Set labdata table: ' + newValue, 'debug', NAME);
            ...
            function update(callback) {
                if ( !callback ) {
                    console.log( 'WARN: missing callback to labdatatable update, stack trace follows: ', new Error().stack );      //  eslint-disable-line no-console
    ...
    /* Module config */
    {
        requires: [ 'dcforms-table-utils', 'edittext-modal' ]
    }
);