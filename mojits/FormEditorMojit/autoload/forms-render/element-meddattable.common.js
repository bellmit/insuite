...
YUI.add(
    ...
    function(Y, NAME) {
        ...
        Y.log('Adding renderer for dcforms meddatatable types.', 'debug', NAME);
        ...
        Y.dcforms.elements.makeMeddataTableRenderer = function(element, creationCallback) {
            ...
            function addRow( values, colWidths, vOffset, rowIdx, callback ) {
                ...
                Y.dcforms.runInSeries( [ addAllColumns, addRowButtons, correctBorderHeights, addHeaderRowBg ], onAllDone );
                ...
                function addRowButtons( itcb ) {
                    if ( 'fill' !== element.page.form.mode || !element.canEdit() || Y.dcforms.isOnServer ) {
                        return itcb( null );
                    }

                    var tempElementButton = Y.dcforms.createSubElement(
            ...
            function renderAbstract(voffset, callback) {
                ...
                generateSubElements( function onSubElementsReady( err ) {
            ...
            function setCellValue( newValue, row, col ) {
                ...
                if ( dataset[row] === -1 || dcCols.cols[col] === -1 ) {
                ...
                generateSubElements( function onSubelementsReady( err ) {
                    if ( err ) {
                        Y.log( 'Could not render table: ' + JSON.stringify( err ), 'error', NAME );
                    }
            ...
            function addNewDataRow( selRow ) {
                ...
                generateSubElements( function onSubElementsReady( err ) {
                    if ( err ) {
                        Y.log( 'Could not generate subelements of table: ' + JSON.stringify( err ), 'error', NAME );
                    }
                    element.page.redraw();
                } );
            }
            ...
            function removeDataRow( idx ) {
                ...
                generateSubElements( function onSubElementsReady( err ) {
                    if ( err ) {
                        Y.log( 'Could not generate subelements for table: ' + JSON.stringify( err ), 'error', NAME );
                    }
                    element.page.redraw();
                } );
            }
